...
async def test_detect_misuse(logger, a_detect_misuse_of_pinjected_proxies):
    ...
    a_misuse_func_misuses = [
        m for m in misuses if m.user_function == "a_misuse_of_injected"
    ]
    assert len(a_misuse_func_misuses) > 0, (
        "Expected misuses in a_misuse_of_injected function"
    )
    ...
    another_misuse_func_misuses = [
        m for m in misuses if m.user_function == "another_misuse"
    ]
    assert len(another_misuse_func_misuses) > 0, (
        "Expected misuses in another_misuse function"
    )

    # Check for misuses in yet_another_misuse function
    yet_another_misuse_func_misuses = [
        m for m in misuses if m.user_function == "yet_another_misuse"
    ]
    assert len(yet_another_misuse_func_misuses) > 0, (
        "Expected misuses in yet_another_misuse function"
    )


@injected_pytest(design_for_test)
async def test_not_detect_imports(logger, a_detect_misuse_of_pinjected_proxies):
    ...
    assert len(misuses) == 0, (
        f"Found {len(misuses)} misuses in valid_module.py, should be 0"
    )


@injected_pytest(design_for_test)
async def test_detect_misuse_in_entrypoint(
    logger, a_detect_misuse_of_pinjected_proxies
):
    ...
    assert len(misuses) == 0, (
        f"Found {len(misuses)} misuses in entrypoint.py, expected 0"
    )
