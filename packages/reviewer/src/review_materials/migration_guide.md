...
# Migration Guide: From __meta_design__ to __design__ in __pinjected__.py

This guide explains how to migrate from the deprecated `__meta_design__` approach to the recommended `__design__` in `__pinjected__.py` files.

## Why Migration is Necessary

The original approach using `__meta_design__` variables directly in module files is being deprecated in favor of a more consistent and maintainable approach using `__design__` variables in dedicated `__pinjected__.py` files.

Benefits of this migration include:
- Clearer file structure and dependency organization
- Improved discovery of configurations by the CLI tool
- Better separation of concerns between code and dependency configuration
- More consistent loading mechanism for designs

## Basic Migration Pattern

### From `__meta_design__` in module files to `__design__` in `__pinjected__.py`

```python
# Before migration: in your_module/__init__.py or your_module/some_file.py
from pinjected import design

__meta_design__ = design(
    overrides=some_design  # Optional override
)

# After migration: in your_module/__pinjected__.py
from pinjected import design
...
```
- your_package
  | __init__.py   [-- contains __meta_design__]
  | module1
  | | __init__.py [-- contains __meta_design__]
  | | util.py     [-- contains __meta_design__]
```
...
When running "python -m pinjected run your_package.module1.util.run", the CLI will:
- Before: Load all `__meta_design__` variables from parent packages
- After: Load all `__design__` variables from `__pinjected__.py` files in parent packages

This new approach provides a more consistent and reliable configuration loading mechanism.
