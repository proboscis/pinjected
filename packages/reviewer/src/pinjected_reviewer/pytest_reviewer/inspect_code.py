...
async def a_collect_imported_symbol_metadata(
    a_collect_symbol_metadata: callable, a_ast: callable, /, src_path: Path
) -> dict[str, SymbolMetadata]:
    ...
    def find_project_root(start_dir: Path) -> tuple[Path, Path | None]:
        ...
        if current == Path("/"):
            logger.debug(
                "  プロジェクトルートが見つかりませんでした。現在のディレクトリを使用します。"
            )
    ...
    def module_path_calc(node: ast.ImportFrom, current_path: Path) -> Path | None:
        ...
        # 外部パッケージならスキップ
        if (
            node.module in external_packages
            or node.module.split(".")[0] in external_packages
        ):
        ...
        if node.level == 0:  # 絶対インポート
            ...
            if is_same_package_import(node.module, current_path):
                ...
                if simple_path.exists():
                    logger.debug(
                        f"  同一ディレクトリ内にモジュールが見つかりました: {simple_path}"
                    )
                    return simple_path
            ...
            for directory in base_dirs:
                ...
                # __init__.pyファイル
                init_path = (
                    directory / Path(node.module.replace(".", "/")) / "__init__.py"
                )
                logger.debug(f"  試行 - {directory} からの__init__パス: {init_path}")
                if init_path.exists():
                    logger.debug(
                        f"  成功: __init__.pyファイルが存在します: {init_path}"
                    )
                    return init_path
        ...
        # 相対インポート
        logger.debug(
            f"  相対インポートの処理: level={node.level}, module={node.module}"
        )
        ...
        for i in range(node.level - 1):
            ...
            logger.debug(
                f"  レベル {i + 1}/{original_level - 1}: {prev_path} -> {relative_path}"
            )
        ...
        if node.module:
            ...
            for i, part in enumerate(module_parts):
                ...
                logger.debug(
                    f"  モジュール部分 {i + 1}/{len(module_parts)}を追加: {prev_path} -> {final_path}"
                )
        ...
        logger.debug(
            f"  相対インポートのパス解決に失敗: level={node.level}, module={node.module}"
        )
        return None
    ...
    for i, node in enumerate(import_nodes):
        logger.debug(
            f"インポート #{i + 1} を処理中: {node.module} (level={node.level})"
        )
        ...
        if module_path and module_path.exists():
            ...
            node.module
            and node.module not in external_packages
            and node.module.split(".")[0] not in external_packages
        ):
            logger.debug(
    ...
    logger.debug(
        f"合計 {len(imported_metadata)} 個のインポートされたシンボルメタデータを収集しました"
    )
...
async def a_detect_misuse_of_pinjected_proxies(
    a_symbol_metadata_getter: callable, a_ast, /, src_path: Path
) -> list[Misuse]:
...
class MisuseDetector(ast.NodeVisitor):
    ...
    def _push_function(self, node):
        self.injection_stack.append(
            FuncStack(node=node, injection_keys=self._get_injection_keys(node))
        )
    ...
    def _innermost_assign_type(self):
        ...
        if assign_node is not None and hasattr(assign_node, "annotation"):
            ...
            if (
                isinstance(assign_node.annotation, ast.Name)
                and "IProxy" in assign_node.annotation.id
            ):
                return True
...
test_not_detect_imports: IProxy = a_detect_misuse_of_pinjected_proxies(
    Path(pinjected_reviewer.__file__).parent.parent
    / "__package_for_tests__"
    / "valid_module.py"
)

test_detect_misuse_3: IProxy = a_detect_misuse_of_pinjected_proxies(
    Path(
        "~/repos/proboscis-ema/src/ema_cython/artemis/label_feature_creation_design.py"
    ).expanduser()
)
test_detect_misuse_4: IProxy = a_detect_misuse_of_pinjected_proxies(
    Path(
        "~/repos/pinjected/packages/reviewer/src/__package_for_tests__/test_review_target.py"
    ).expanduser()
)
...
check_symtable: IProxy = a_symtable(
    Path(pinjected_reviewer.examples.__file__)
).get_identifiers()
