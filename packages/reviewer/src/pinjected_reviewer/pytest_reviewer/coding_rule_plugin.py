...
async def run_review_for_pytest(session: pytest.Session):
    ...
    if only_changed_files:
        ...
        d = await mc.a_final_design + design(pytest_session=session)
    ...
    if has_errors and not session.config.getoption("pinjected_continue_on_error"):
        ...
        for diagnostic in [d for d in diagnostics if d.level == "error"]:
            ...
            # Log the error
            logger.error(
                f"{file_path}{line_info} - {diagnostic.name}: {diagnostic.message}"
            )
...
def pytest_terminal_summary(terminalreporter):
    ...
    if not config.getoption("pinjected_reviewer"):
        terminalreporter.write_line(
            "pinjected-reviewer: Disabled by --no-pinjected-reviewer option"
        )
        return
    ...
    for diagnostic in diagnostics:
        ...
        if diagnostic.level == "error":
            terminalreporter.write_line(
                f"ERROR: {location} - {diagnostic.name}", red=True
            )
        elif diagnostic.level == "warning":
            terminalreporter.write_line(
                f"WARNING: {location} - {diagnostic.name}", yellow=True
            )
        elif diagnostic.level == "suggest":
            terminalreporter.write_line(
                f"SUGGESTION: {location} - {diagnostic.name}", cyan=True
            )
        elif diagnostic.level == "approve":
            terminalreporter.write_line(
                f"APPROVED: {location} - {diagnostic.name}", green=True
            )
