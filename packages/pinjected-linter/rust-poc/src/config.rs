...
pub fn merge_config(
    ...
    if let Some(cfg) = config {
        ...
        if !cfg.disable.is_empty() && cli_enable.is_empty() && cli_disable.is_empty() {
            ...
            let all_rules = vec![
                "PINJ001", "PINJ002", "PINJ003", "PINJ004", "PINJ005",
                "PINJ006", "PINJ007", "PINJ009", "PINJ010",
                "PINJ011", "PINJ012", "PINJ013", "PINJ014", "PINJ015",
                "PINJ016", "PINJ017",
            ];
            let enabled: Vec<String> = all_rules.into_iter()
    ...
    if !cli_enable.is_empty() {
        ...
        let all_rules = vec![
            "PINJ001", "PINJ002", "PINJ003", "PINJ004", "PINJ005",
            "PINJ006", "PINJ007", "PINJ008", "PINJ009", "PINJ010",
            "PINJ011", "PINJ012", "PINJ013", "PINJ014", "PINJ015",
        ];
        let enabled: Vec<String> = all_rules.into_iter()
    ...
    let defaults = vec![
        ".venv", "venv", "__pycache__", ".git", ".tox", "build", "dist",
...
mod tests {
    ...
    fn test_find_pyproject_toml() {
        ...
        fs::write(&pyproject_path, "[tool.pinjected-linter]\nexclude = [\"test\"]").unwrap();
        
        // From the same directory
        assert_eq!(find_pyproject_toml(dir.path()), Some(pyproject_path.clone()));
    ...
    fn test_find_config_pyproject_toml() {
        ...
        fs::write(&parent_toml, "[tool.pinjected-linter]\nexclude = [\"test\"]").unwrap();
