...
impl IProxyAnnotationsRule {
    ...
    fn is_service_type(&self, annotation: &Expr) -> bool {
        ...
        service_patterns
            .iter()
            .any(|pattern| annotation_str.contains(pattern))
    }
    ...
    fn check_instance_function(
        ...
        if let Some(return_annotation) = &func.returns {
            ...
            if self.is_service_type(return_annotation)
                && !self.is_iproxy_annotation(return_annotation)
            {
    ...
    fn check_instance_function_async(
        &self,
