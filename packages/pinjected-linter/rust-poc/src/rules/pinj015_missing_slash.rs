...
impl MissingSlashRule {
    ...
    fn has_no_dependencies_marker(func: &rustpython_ast::StmtFunctionDef) -> bool {
        ...
        if let Some(docstring) = Self::get_docstring(func) {
            if docstring.to_lowercase().contains("pinjected: no dependencies") 
                || docstring.to_lowercase().contains("pinjected:no dependencies") {
                return true;
            }
        }
    ...
    fn generate_suggestion(func_name: &str, args: &rustpython_ast::Arguments) -> String {
        ...
        for arg in &args.args {
            ...
            let is_dependency = dependency_patterns.iter().any(|pattern| arg_lower.contains(pattern))
        ...
        if !likely_deps.is_empty() && !likely_runtime.is_empty() {
            ...
            let all_args: Vec<String> = args.args.iter()
...
"#;
        ...
        assert!(violations[0].message.contains("add 'pinjected: no dependencies'"));
