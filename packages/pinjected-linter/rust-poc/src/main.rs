...
use pinjected_linter::{lint_path, find_python_files, LinterOptions};
use pinjected_linter::location::LineIndex;
use pinjected_linter::config::{load_config, merge_config, find_config_pyproject_toml};
...
struct Args {
    ...
    #[arg(short = 's', long = "severity", value_enum, conflicts_with = "show_only")]
...
fn main() -> Result<()> {
    ...
    let config = if let Some(config_path) = &args.config {
        ...
        let abs_path = start_path.canonicalize().unwrap_or_else(|_| start_path.to_path_buf());
        if args.verbose {
            eprintln!("Searching for pyproject.toml from: {}", abs_path.display());
        }
        if let Some(pyproject) = find_config_pyproject_toml(&abs_path) {
            if args.verbose {
                eprintln!("Found pyproject.toml with [tool.pinjected-linter] at: {}", pyproject.display());
            }
    ...
    let (enable_rules, skip_patterns) = merge_config(
        config.as_ref(),
        &args.enable,
        &args.disable,
        &args.skip,
    ...
    for path_str in &args.paths {
        ...
        if args.verbose {
            ...
            eprintln!("Found {} Python files to analyze in {}", files.len(), path.display());
        }
    ...
    if let Some(min_severity) = args.severity {
        ...
        all_violations = all_violations.into_iter()
            .map(|(file, violations)| {
                let filtered: Vec<_> = violations.into_iter()
        ...
        for level in &args.show_only {
            match level.to_lowercase().as_str() {
                "error" | "errors" => { show_levels.insert(Severity::Error); }
                "warning" | "warnings" => { show_levels.insert(Severity::Warning); }
                "info" => { show_levels.insert(Severity::Info); }
                _ => {
    ...
    show_statistics(&all_violations, total_files, elapsed.as_secs_f64(), use_color, filter_applied);
    
    // Determine exit code based on what happened
    let exit_code = if had_parse_errors {
        eprintln!("\nExiting with code {} due to parse errors", exit_codes::PARSE_ERROR);
        exit_codes::PARSE_ERROR
    } else if had_file_errors {
        eprintln!("\nExiting with code {} due to file errors", exit_codes::FILE_ERROR);
        exit_codes::FILE_ERROR
    } else {
        // Check for violations
        let has_errors = all_violations.iter()
            .any(|(_, violations)| violations.iter().any(|v| matches!(v.severity, pinjected_linter::models::Severity::Error)));
        let has_warnings = all_violations.iter()
            .any(|(_, violations)| violations.iter().any(|v| matches!(v.severity, pinjected_linter::models::Severity::Warning)));
...
fn show_statistics(
    ...
    if total_violations == 0 {
        ...
        eprintln!("Linting Summary{}", if filter_applied { " (filtered)" } else { "" });
        eprintln!("{}", "-".repeat(60));
...
fn report_terminal(violations: &[(std::path::PathBuf, Vec<pinjected_linter::models::Violation>)], show_source: bool, use_color: bool) -> Result<()> {
    use pinjected_linter::models::Severity;
    ...
    for (file_idx, (file, file_violations)) in sorted_violations.iter().enumerate() {
        ...
        if use_color {
            ...
            print!(" (");
            if file_errors > 0 {
                print!("\x1b[31m{} error{}\x1b[0m", file_errors, if file_errors == 1 { "" } else { "s" });
                if file_warnings > 0 || file_infos > 0 {
                    print!(", ");
                }
            }
            if file_warnings > 0 {
                print!("\x1b[33m{} warning{}\x1b[0m", file_warnings, if file_warnings == 1 { "" } else { "s" });
                if file_infos > 0 {
                    print!(", ");
                }
            }
            ...
            println!("{} ({} error{}, {} warning{}, {} info)",
                file.display(),
                file_errors, if file_errors == 1 { "" } else { "s" },
                file_warnings, if file_warnings == 1 { "" } else { "s" },
                file_infos
            );
        }
        ...
        if let Ok(content) = fs::read_to_string(&file) {
            ...
            for violation in sorted_file_violations {
                ...
                if use_color {
                    ...
                    println!("  {}:{}: {}: {}", line, column, violation.rule_id, violation.message);
                }
...
fn report_json(violations: &[(std::path::PathBuf, Vec<pinjected_linter::models::Violation>)]) -> Result<()> {
    use serde_json::json;
...
fn report_github(violations: &[(std::path::PathBuf, Vec<pinjected_linter::models::Violation>)]) -> Result<()> {
