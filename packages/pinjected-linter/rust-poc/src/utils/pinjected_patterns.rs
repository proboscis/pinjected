...
use rustpython_ast::{Expr, StmtFunctionDef, StmtAsyncFunctionDef, Keyword};
...
pub fn has_instance_callable_decorator(func: &StmtFunctionDef) -> bool {
    func.decorator_list.iter().any(|d| is_instance_callable_decorator(d))
}
...
pub fn is_noun_like(name: &str) -> bool {
    const VERB_PREFIXES: &[&str] = &[
        "get_", "set_", "create_", "make_", "build_", 
        "fetch_", "load_", "save_", "delete_", "update_",
        "process_", "handle_", "parse_", "validate_", "check_",
        "compute_", "calculate_", "generate_", "render_", "execute_"
    ];
...
pub fn is_verb_like(name: &str) -> bool {
    ...
    const VERB_PREFIXES: &[&str] = &[
        "get_", "set_", "create_", "build_", "setup_", "initialize_", "make_",
        "fetch_", "load_", "save_", "delete_", "update_", "process_", "handle_",
        "execute_", "run_", "start_", "stop_", "open_", "close_", "connect_",
        "disconnect_", "prepare_", "generate_", "compute_", "calculate_",
        "validate_", "check_", "send_", "receive_", "parse_", "format_",
        "convert_", "transform_", "register_", "unregister_", "enable_",
        "disable_", "configure_", "mount_", "unmount_", "bind_", "unbind_",
        "attach_", "detach_", "compile_", "render_", "draw_", "write_", "read_",
        "scan_", "authenticate_", "authorize_", "verify_", "sign_", "encrypt_",
        "decrypt_",
    ];
    ...
    const STANDALONE_VERBS: &[&str] = &[
        "init", "initialize", "setup", "build", "create", "make", "get", "fetch",
        "load", "process", "execute", "run", "validate", "check", "verify",
        "authenticate", "authorize", "parse", "format", "render", "compile",
        "transform", "start", "stop", "pause", "resume", "reset", "refresh",
        "handle", "manage", "control", "operate", "perform", "workflow",
        "pipeline", "orchestrate", "coordinate", "filter", "sort", "group",
        "aggregate", "reduce", "publish", "subscribe", "broadcast", "notify",
        "alert",
    ];
    ...
    const NOUN_SUFFIXES: &[&str] = &[
        "_data", "_info", "_config", "_configuration", "_result", "_response",
        "_request", "_state", "_status", "_context", "_manager", "_handler",
        "_service", "_client", "_provider", "_factory", "_builder", "_validator",
        "_processor", "_controller",
    ];
