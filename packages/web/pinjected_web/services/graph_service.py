...
class DIGraphService:
    ...
    def get_graph(self, module_path: str, root_key: Optional[str] = None) -> GraphResponse:
        """
        ...
        try:
            ...
            raise ValueError(f"Failed to generate graph for {module_path}: {str(e)}")
    ...
    def get_node_details(self, module_path: str, node_key: str) -> NodeDetails:
        ...
        try:
            ...
            all_edges = digraph.to_edges("__root__", list(digraph.explicit_mappings.keys()))
            ...
            import traceback
            print(f"Error in get_node_details: {str(e)}")
            print(traceback.format_exc())
            raise ValueError(f"Failed to get node details for {node_key} in {module_path}: {str(e)}")
    ...
    def search(self, module_path: str, query: str) -> SearchResponse:
        ...
        try:
            ...
            for edge in all_edges:
                ...
                try:
                    if edge.metadata and hasattr(edge.metadata, "unwrap") and edge.metadata.unwrap():
                        metadata = edge.metadata.unwrap()
                        if hasattr(metadata, "docstring") and metadata.docstring:
                            if query.lower() in metadata.docstring.lower():
                                results.append(edge.key)
                                continue
                except Exception:
                    pass
                ...
                try:
                    if edge.spec and hasattr(edge.spec, "unwrap") and edge.spec.unwrap():
            ...
            print(traceback.format_exc())
            raise ValueError(f"Failed to search dependencies in {module_path}: {str(e)}")
    ...
    def _format_metadata(self, metadata) -> Optional[MetadataInfo]:
        ...
        try:
            ...
            return MetadataInfo(
                location=location,
                docstring=metadata_obj.docstring if hasattr(metadata_obj, "docstring") else None,
                source=str(metadata_obj.source) if hasattr(metadata_obj, "source") else None
            )
        except Exception:
            return None
...
def service(connection):
    ...
    return f"Service using {connection}" """
            ...
            import traceback
            print(f"Error getting source code for {node_key}: {str(e)}")
            print(traceback.format_exc())
            return "Source code not available"
