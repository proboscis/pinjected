...
def load_module_var(module_path: str) -> Any:
    ...
    if len(parts) < 2:
        raise ValueError(
            f"Invalid module path: {module_path}. Expected format: module.submodule.variable"
        )
    ...
    try:
        ...
        raise ImportError(f"Failed to import module {module_name}: {e!s}")
    except AttributeError as e:
        raise AttributeError(
            f"Failed to find variable {var_name} in module {module_name}: {e!s}"
        )
