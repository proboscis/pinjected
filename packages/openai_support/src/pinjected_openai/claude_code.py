...
async def a_claude_code_subprocess(
    ...
    **kwargs,
    ...
    logger.info(f"Executing Claude Code with prompt length: {len(prompt)}, using command: {claude_cmd}")
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            logger.error(f"Claude Code failed with return code {process.returncode}: {error_msg}")
        ...
        logger.error("Claude Code command not found. Make sure @anthropic-ai/claude-code is installed")
        raise ClaudeCodeNotFoundError(
            "Claude Code command not found. Install with: npm install -g @anthropic-ai/claude-code"
        ) from e
...
async def a_sllm_claude_code(  # noqa: PINJ045
    ...
    if images is not None:
        logger.warning("Images are not currently supported in Claude Code subprocess implementation")
