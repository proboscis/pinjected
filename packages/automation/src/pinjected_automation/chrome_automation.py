...
def take_chrome_screenshot(
...
) -> tuple[PIL.Image.Image, BrowserBBox]:
    """
    ...
    try:
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp_file:
            ...
            logger.info(
                f"cropped image resolution and original: {final_image.size} {image.size}"
            )
            image.close()
...
class AnthropicToolResult:
    ...
    def add_text(self, text: str):
        return replace(self, content=self.content + [{"type": "text", "text": text}])

    def add_image(self, img: PIL.Image.Image):
        return replace(
            self, content=self.content + [self._pil_image_to_image_block(img)]
        )
...
def new_AnthropicToolResult(pil_image_to_image_block, /, *args, **kwargs):
    return AnthropicToolResult(pil_image_to_image_block, *args, **kwargs)
...
class AnthropicChatContext:
    ...
    def validate(self):
        for c in self.messages:
            ...
            assert isinstance(c["content"], list), (
                f"content is not a list: {c['content']}"
            )
    ...
    def add_tool_result(self, tool_result: AnthropicToolResult):
        return replace(
            self,
            messages=self.messages
            + [{"role": "user", "content": [tool_result.to_tool_result_block()]}],
        )
    ...
    def add_image_message(self, role: str, img: PIL.Image.Image):
        ...
        return replace(
            self,
            messages=self.messages
            + [{"role": role, "content": [self._pil_image_to_image_block(img)]}],
        )
...
async def a_add_chrome_screenshot_cxt(
    take_chrome_screenshot, /, cxt: AnthropicChatContext
):
    screenshot, bbox = take_chrome_screenshot(include_cursor=True)
    assert screenshot.size == (bbox.width, bbox.height), (
        f"Screenshot size mismatch: {screenshot.size} vs {bbox}"
    )
...
async def a_chrome_screenshot(
    take_chrome_screenshot, new_AnthropicToolResult, /, tool_id: str
):
    screenshot, bbox = take_chrome_screenshot(include_cursor=True)
...
def new_AnthropicChatContext(pil_image_to_image_block, anthropic_client, /, **kwargs):
    return AnthropicChatContext(pil_image_to_image_block, anthropic_client, **kwargs)


@injected
def set_chrome_window_size(
    width: int, height: int, x: int | None = None, y: int | None = None
) -> BrowserBBox:
    """
    ...
    match tool_use.input:
        case {"command": "create", "path": path, "file_text": file_text}:
            ...
            cxt = cxt.add_tool_result(
                new_AnthropicToolResult(tool_use.id).add_text(f"Created file at {path}")
            )
        case {"command": "view", "path": path}:
            ...
            if path.is_file():
                ...
                cxt = cxt.add_tool_result(
                    new_AnthropicToolResult(tool_use.id).add_text(path.read_text())
                )
        case {"command": "str_replace", "path": path, "old_str": old, "new_str": new}:
            ...
            Path(path).with_suffix(
                f".{pd.Timestamp.now().strftime('%Y%m%d%H%M%S')}.bak"
            ).write_text(text)
            new_text = text.replace(old, new)
            Path(path).write_text(new_text)
            logger.warning(f"Replaced {old} with {new} in {path}")
            cxt = cxt.add_tool_result(
                new_AnthropicToolResult(tool_use.id).add_text(
    ...
    match tool_use.input:
        case {"command": command}:
            ...
            try:
                ...
                cxt = cxt.add_tool_result(
                    new_AnthropicToolResult(tool_use.id).add_text(result.stdout)
                )
            except subprocess.CalledProcessError as e:
                cxt = cxt.add_tool_result(
                    new_AnthropicToolResult(tool_use.id).add_text(e.stderr)
                )
        case _:
...
async def a_handle_action_loop(
    ...
    def show_usage():
        ...
        rich.print(
            f"[bold orange]Cost: {cost_usd:.2f} USD, Cumulative: {cum_price_usd:.2f} USD[/bold orange]"
        )
    ...
    async def handle_block(cnt) -> AnthropicChatContext:
        ...
        match cnt:
            ...
            case BetaToolUseBlock(id=_id, input=_input, name="wait", type=_) as use:
                ...
                cxt = cxt.add_tool_result(
                    new_AnthropicToolResult(_id).add_text(
            ...
            case (
                BetaToolUseBlock(
                    id=_id, input=_input, name="str_replace_editor", type=_
                ) as use
            ):
            ...
            case BetaToolUseBlock(id=_id, input=_input, name="computer", type=_) as use:
                ...
                match _input:
                    case {"action": "mouse_move", "coordinate": (x, y)}:
                        ...
                        tool_result = new_AnthropicToolResult(tool_id=_id).add_text(
                    ...
                    case {"action": "right_click", "coordinate": (x, y)}:
                        screen_x, screen_y = last_bbox.to_screen_coordinates(x, y)
                        logger.info(
                            f"Mouse right click at {x},{y} ({screen_x},{screen_y})"
                        )
                        cxt = cxt.add_tool_result(
                            new_AnthropicToolResult(_id).add_text(
                                f"Mouse right clicked at {x},{y}"
                            )
                        )
                    ...
                    case {"action": "type", "text": text}:
                        ...
                        cxt = cxt.add_tool_result(
                            new_AnthropicToolResult(_id).add_text(f"Typed: {text}")
                        )
                    ...
                    case {"action": "key", "text": key_combination} | {
                        ...
                        cxt = cxt.add_tool_result(
                            new_AnthropicToolResult(_id).add_text(
                                f"Keyboard input: {key_combination}"
                            )
                        )
                    case _:
    ...
    while True:
        ...
        try:
            ...
            if "This action is restricted" in str(e):
                raise AnthropicAutomationDown(
                    f"Anthropic seems to be down: {cxt.truncate_image_data()}"
                ) from e
            raise AnthropicAutomationException(
                f"Error in calling Anthropic: {cxt.truncate_image_data()}"
            ) from e
        ...
        for block_i, cnt in enumerate(resp.content):
            try:
                ...
                raise AnthropicAutomationException(
                    f"Error in handling block({block_i}): {cnt} in {cxt.truncate_image_data()}"
                ) from e
