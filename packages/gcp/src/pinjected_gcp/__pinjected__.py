...
__design_spec__ = DesignSpec.new(
    gcp_service_account_credentials=SimpleBindSpec(
        validator=lambda item: "gcp_service_account_credentials must be a dict"
    ...
    gcp_storage_client=SimpleBindSpec(
        validator=lambda item: "gcp_storage_client must be a storage.Client"
        if not isinstance(item, storage.Client)
        else None,
        documentation="Google Cloud Storage client instance for interacting with GCS",
    ),
    logger=SimpleBindSpec(
        validator=lambda item: "logger must be a Logger instance"
        if not isinstance(item, Logger)
        else None,
        documentation="Logger instance for logging operations",
    ),
    a_upload_gcs=SimpleBindSpec(
        validator=lambda item: "a_upload_gcs must be a callable"
    ...
    a_download_gcs=SimpleBindSpec(
        validator=lambda item: "a_download_gcs must be a callable"
        if not callable(item)
        else None,
        documentation="""
