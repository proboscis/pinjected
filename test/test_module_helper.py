...
def test_walk_module_with_special_files_multiple_attrs():
    ...
    assert "__pinjected__" in sources, (
        "Should find special_config from __pinjected__.py"
    )
...
def test_no_duplicates():
    ...
    assert len(paths) == len(unique_paths), (
        f"Found duplicate attributes: {len(paths) - len(unique_paths)} duplicates"
    )
    ...
    for i, item in enumerate(items):
        ...
        if "__design__" in item.var_path:
            ...
            for j in range(i + 1, len(items)):
                ...
                if (
                    module_path == compare_module
                    and "special_config" in compare_item.var_path
                ):
            ...
            for j in range(i):
                ...
                if (
                    ...
                    assert False, (
                        f"Incorrect order: {compare_item.var_path} came before {item.var_path}"
                    )
...
def test_multiple_attr_names_complex_order():
    ...
    assert found_multiple_attrs, (
        "No modules with multiple attributes found to test ordering"
    )
    ...
    if "special_config" in reverse_attr_types and "__design__" in reverse_attr_types:
        ...
        first_special_config = (
            ...
            if "special_config" in reverse_attr_types
            else -1
        )
        first_meta_design = (
