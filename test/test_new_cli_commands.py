...
def test_injected_to_idea_configs_includes_new_commands(injected_to_idea_configs):
    ...
    assert any("describe_json" in name for name in config_names), (
        f"describe_json not found in {config_names}"
    )
    assert any("trace" in name for name in config_names), (
        f"trace not found in {config_names}"
    )
    assert any("list module" in name for name in config_names), (
        f"list module not found in {config_names}"
    )
    ...
    for cfg in result.configs["test_var"]:
        if "describe_json" in cfg.name:
            ...
            assert "test_var" in " ".join(cfg.arguments[1:]), (
            ...
            assert cfg.arguments[1] == "test_var", (
            ...
            assert cfg.arguments[1] == "test_module", "list should have module path"
...
def test_list_command_uses_module_path(injected_to_idea_configs):
    ...
    for var_path, expected_module in test_cases:
        ...
        list_configs = [
            cfg
            for cfg in result.configs[var_path.split(".")[-1]]
