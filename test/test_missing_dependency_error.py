...
def test_missing_dependency_exception_message():
    ...
    failures = [
        DependencyResolutionFailure(
            key="key1", trace=["root", "dep1", "key1"], cause="Key not found"
        ),
        DependencyResolutionFailure(
            key="key2", trace=["root", "dep2", "key2"], cause="Key not found"
        ),
    ]
    ...
    assert (
        "Missing Dependencies: {'key1', 'key2'}" in message
        or "Missing Dependencies: {'key2', 'key1'}" in message
    )
    ...
    assert (
        "Use the 'describe' command for more detailed dependency information" in message
    )
...
def test_integration_with_resolver():
    ...
    assert any(
        ...
        for pattern in ["Missing Dependencies", "Cyclic Dependencies"]
    )
