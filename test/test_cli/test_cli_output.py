...
async def test_command_help_documentation():
    ...
    assert "load the injected variable" in run.__doc__, (
        "The 'run' command documentation should explain its purpose"
    )

    assert describe.__doc__ is not None, (
        "The 'describe' command should have documentation"
    )
    assert "human-readable description" in describe.__doc__, (
        "The 'describe' command documentation should explain its purpose"
    )
...
def test_cli_command_visibility_with_subprocess():
    ...
    assert hasattr(cli, "describe"), (
        "The 'describe' command should be registered in CLI"
    )
...
def test_run_command_help_with_subprocess():
    ...
    assert "var_path" in output, (
        "The 'run' command help should mention var_path parameter"
    )
    assert "design_path" in output, (
        "The 'run' command help should mention design_path parameter"
    )
...
def test_describe_command_help_with_subprocess():
    ...
    assert "var_path" in output, (
        "The 'describe' command help should mention var_path parameter"
    )
...
def test_command_visibility_in_module():
    ...
    try:
        ...
        assert hasattr(main_impl, "main"), (
            "The main_impl module should have a main function"
        )

        assert hasattr(main_impl, "run"), (
            "The 'run' command should be available in main_impl"
        )
        assert hasattr(main_impl, "describe"), (
            "The 'describe' command should be available in main_impl"
        )
