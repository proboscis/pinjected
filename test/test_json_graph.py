...
def test_to_json_single_root():
    ...
    main_target_edge = next(
        (edge for edge in edges if edge["key"] == "main_target"), None
    )
...
def test_to_json_replace_missing():
    ...
    incomplete_design = design(dep2=dep2, main_target=main_target)
    ...
    assert len(json_output_with_replace["edges"]) >= len(
        json_output_without_replace["edges"]
    )
