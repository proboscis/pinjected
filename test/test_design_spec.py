...
async def test_design_spec_get_spec():
    ...
    test_spec = DesignSpec.new(
        test_key=SimpleBindSpec(
            validator=lambda item: "test_key must be int"
...
async def test_design_spec_addition():
    ...
    spec1 = DesignSpec.new(key1=SimpleBindSpec(documentation="Spec 1"))

    spec2 = DesignSpec.new(key2=SimpleBindSpec(documentation="Spec 2"))
...
async def test_design_spec_precedence():
    ...
    reversed_doc = await reversed_spec.spec_doc_provider.unwrap()(
        StrBindKey("shared_key")
    )
...
async def test_multi_level_design_spec_hierarchy():
    ...
    found_top_level = any(
        "test_package.__pinjected__" in spec_path for spec_path in specs_found
    )
    found_child_level = any(
        "test_package.child.__pinjected__" in spec_path for spec_path in specs_found
    )
    ...
    assert design_name_spec is not Nothing, (
        "Should find spec for design_name from top level"
    )

    design_var_spec = child_spec.get_spec(StrBindKey("design_var"))
    assert design_var_spec is not Nothing, (
        "Should find spec for design_var from child level"
    )
    ...
    merged_spec = (
        child_spec + top_spec
    )  # In the current implementation, child_spec takes precedence for duplicate keys
    ...
    assert "Child module" in unsafe_perform_io(doc.unwrap()), (
        "Should have documentation from child directory"
    )
