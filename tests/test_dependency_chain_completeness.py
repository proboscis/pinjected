...
def test_collect_dependencies_includes_all_keys():
    ...
    assert sorted(deps_map["service"]) == sorted(["middle1", "middle2"]), "service should depend on middle1 and middle2"
...
def test_build_edges_includes_all_keys():
    ...
    assert set(edge_dict.keys()) == expected_keys, f"Missing keys: {expected_keys - set(edge_dict.keys())}"
    
    assert sorted(edge_dict["root"].dependencies) == ["service"], "root should depend on service"
    assert sorted(edge_dict["service"].dependencies) == sorted(["middle1", "middle2"]), "service should depend on middle1 and middle2"
    assert edge_dict["middle1"].dependencies == ["leaf1"], "middle1 should depend on leaf1"
    assert edge_dict["middle2"].dependencies == ["leaf2"], "middle2 should depend on leaf2"
    ...
    assert edge_dict["middle1"].used_by == ["service"], "middle1 should be used by service"
    assert edge_dict["middle2"].used_by == ["service"], "middle2 should be used by service"
...
def test_leaf_nodes_included_in_edges():
    ...
    assert leaf_keys == expected_leaf_keys, f"Missing leaf keys: {expected_leaf_keys - leaf_keys}"
