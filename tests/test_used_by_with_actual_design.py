...
def test_used_by_in_dependency_graph_description():
    ...
    mock_edges = [
        ...
        EdgeInfo(key="service1", dependencies=["dep1", "shared_dep"], used_by=["root"], metadata=Nothing, spec=Nothing),
        EdgeInfo(key="service2", dependencies=["dep2", "shared_dep"], used_by=["root"], metadata=Nothing, spec=Nothing),
        EdgeInfo(key="dep1", dependencies=[], used_by=["service1"], metadata=Nothing, spec=Nothing),
        EdgeInfo(key="dep2", dependencies=[], used_by=["service2"], metadata=Nothing, spec=Nothing),
        EdgeInfo(key="shared_dep", dependencies=[], used_by=["service1", "service2"], metadata=Nothing, spec=Nothing)
    ]
        ...
        for key in ["shared_dep", "dep1", "dep2", "service1", "service2"]:
            ...
            for call in panel_calls:
                ...
                if isinstance(panel.title, Text) and panel.title.plain == key:
                    ...
                    assert "Used by:" in panel_content, f"Used by field missing for {key}"
                    
                    if key == "shared_dep":
                        assert "service1" in panel_content and "service2" in panel_content, \
