...
def test_build_edges_with_complex_dependency_graph():
    ...
    assert sorted(edge_dict["shared_dep"].used_by) == sorted(
        ["service1", "service2", "service3"]
    ), "shared_dep should be used by service1, service2, and service3"
    ...
    assert sorted(edge_dict["service1"].used_by) == ["root"], (
        "service1 should be used by root"
    )
    assert sorted(edge_dict["service2"].used_by) == ["root"], (
        "service2 should be used by root"
    )
    assert sorted(edge_dict["service3"].used_by) == ["root"], (
        "service3 should be used by root"
    )
