...
def modify_class(class_ast, method_names, attrs_to_replace):
    for node in class_ast.body:
        if isinstance(node, ast.FunctionDef) and node.name in method_names:
            raise RuntimeError(
                f"Method {node.name} must be an async method in order to be injected! but it is sync method."
            )
...
def convert_cls(cls, methods_to_convert: list[str], attrs_to_replace: list[str]):
    ...
    modified_class_ast = modify_class(
        class_node, method_names=methods_to_convert, attrs_to_replace=attrs_to_replace
    )
...
if __name__ == "__main__":
    convert_cls(
        PClassExample, ["simple_method", "method1", "method2"], ["_dep1", "_dep2", "c"]
    )
