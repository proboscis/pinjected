...
class ConfigCreationArgs:
    ...
    def to_design(self):
        ...
        design_obj = design(
            ...
            runner_script_path=Injected.bind(lambda: self.runner_script_path or __file__),
            interpreter_path=Injected.bind(lambda: self.interpreter_path or sys.executable),
            default_design_paths=Injected.bind(lambda: find_default_design_paths(self.module_path, self.default_design_path)),
            default_working_dir=Injected.bind(lambda project_root: maybe(lambda: self.working_dir)() | Some(
                str(project_root))),
            default_design_path=Injected.bind(lambda default_design_paths: default_design_paths[0]),
            logger=logger,
        ...
        logger.info(f"custom_idea_config_creator:{design_obj['custom_idea_config_creator']}")
        return design_obj
