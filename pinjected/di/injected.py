...
def extract_dependency(dep: str | type | Callable | Injected | DelegatedVar[Injected]) -> set[str]:
    if isinstance(dep, str):
        return {dep}
    if isinstance(dep, type):  # it's a constructor and we must use __init__ for backward compatibility.
