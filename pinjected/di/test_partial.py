...
def target_function(d1, d2, d3, /, a, b, *args, named=4, **kwargs):
    return frozendict(
        d1=d1, d2=d2, d3=d3, a=a, b=b, args=args, named=named, kwargs=kwargs
    )
...
wrapped = Partial(
    ...
    injection_targets=dict(
        d1=Injected.pure(1), d2=Injected.pure(2), d3=Injected.pure(3)
    ),
)
target_result = target_function(
    1, 2, 3, "a", "b", "args1", "args2", named=5, kw1="kw1", kw2="kw2"
)
...
def test_with_injected_decorator():
    ...
    d = design(d1=1, d2=2, d3=3)
    assert (
        d.provide(wrapped("a", "b", "args1", "args2", named=5, kw1="kw1", kw2="kw2"))
        == target_result
    )
