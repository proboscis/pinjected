...
def validate_path_under_root(file_path: Path, root_path: Path) -> None:
    ...
    if not str(file_path).startswith(str(root_path)):
        raise ModulePathError(
            f"File path {file_path} is not under root path {root_path}"
        )

    logger.trace(f"Validated file path {file_path} is under root path {root_path}")


def normalize_special_filenames(special_filenames: str | list[str] | None) -> list[str]:
    """
    ...
    if isinstance(special_filenames, list):
        ...
        return special_filenames
    raise TypeError(
        f"special_filenames must be a string, list of strings, or None, got {type(special_filenames)}"
    )
...
def validate_module_paths(file_path: Path, root_module_path: Path) -> None:
    ...
    if not isinstance(root_module_path, Path):
        raise TypeError(
            f"root_module_path must be a Path object, got {type(root_module_path)}"
        )
    ...
    if not root_module_path.is_dir():
        raise ModulePathError(
            f"Root module path is not a directory: {root_module_path}"
        )
...
def get_relative_path(file_path: Path, root_module_path: Path) -> Path:
    ...
    try:
        ...
        raise ModulePathError(
            f"File path {file_path} is not under root module path {root_module_path}"
        )
...
def get_module_name(file_path: Path, root_module_path: Path) -> str:
    ...
    if str(relative_path).startswith("src/"):
        logger.trace("File path starts with src/, removing src/ prefix")
...
) -> tuple[ModuleType, importlib.machinery.ModuleSpec]:
    """
    ...
    try:
        ...
        if isinstance(e, ValueError):
            raise ModuleLoadError(
                f"Cannot execute module {module_name} at {file_path}: {e}"
            )
        if isinstance(e, (ImportError, ModuleNotFoundError)):
            raise ModuleLoadError(
                f"Cannot import module {module_name} at {file_path}: {e}"
            )
        raise ModuleLoadError(
            f"Unexpected error ({error_type}) when loading module {module_name} at {file_path}: {e}"
        )


def extract_module_attribute(
    module: ModuleType, module_name: str, attr_name: str
) -> ModuleVarSpec | None:
    """
...
def build_parent_path_tree(current_path: Path, root_module_path: Path) -> list[Path]:
    """
    ...
    while str(current_path) != str(root_module_path):
        ...
        if not str(current_path).startswith(str(root_module_path)) and str(
            current_path
        ) != str(root_module_path):
            raise ModulePathError(
                f"Invalid path hierarchy: {current_path} is not under {root_module_path}"
            )
...
def build_module_hierarchy(file_path: Path, root_module_path: Path) -> ModuleHierarchy:
    ...
    return ModuleHierarchy(root_module_path=root_module_path, module_paths=path_tree)


def find_special_files(
    directory: Path, special_filenames: list[str], exclude_path: Path | None = None
) -> list[Path]:
    """
    ...
    if not isinstance(special_filenames, list):
        raise TypeError(
            f"special_filenames must be a list, got {type(special_filenames)}"
        )

    for i, filename in enumerate(special_filenames):
        if not isinstance(filename, str):
            raise TypeError(
                f"special_filenames[{i}] must be a string, got {type(filename)}"
            )
    ...
    for filename in special_filenames:
        if not filename:
            logger.warning("Empty filename in special_filenames - skipping")
            continue
...
def process_special_file(
    special_file_path: Path, root_module_path: Path, attr_names: list[str]
    ...
    if not isinstance(special_file_path, Path):
        raise TypeError(
            f"special_file_path must be a Path object, got {type(special_file_path)}"
        )
    ...
    for attr_name in attr_names:
        attr_spec = extract_module_attribute(
            special_module, special_module_name, attr_name
        )
        if attr_spec:
...
def process_directory(params: DirectoryProcessParams) -> Iterator[ModuleVarSpec]:
    """
    ...
    special_files = find_special_files(
        params.directory, params.special_filenames, params.exclude_path
    )
    ...
    for special_file_path in special_files:
        ...
        try:
            attr_specs = process_special_file(
                special_file_path, params.root_module_path, params.attr_names
            )
            ...
            if attr_specs:
                ...
                for attr_name in params.attr_names:
                    if attr_name in attr_specs:
                        logger.debug(
                            f"Found attribute {attr_name} in {special_file_path}"
                        )
                        results_found = True
                        yield attr_specs[attr_name]
                    else:
                        logger.debug(
                            f"Attribute {attr_name} not found in {special_file_path}"
                        )
            else:
    ...
    if not results_found:
        logger.info(
            f"No attributes named {params.attr_names} found in any special files in directory {params.directory}"
        )


def process_parent_module(
    parent_file_path: Path, attr_names: list[str], root_module_path: Path
) -> dict[str, ModuleVarSpec]:
    """
...
def walk_parent_modules(
    file_path: Path, attr_names: list[str], root_module_path: Path
    ...
    if parent_file_path.exists() and parent_file_path != file_path:
        ...
        attr_specs = process_parent_module(
            parent_file_path, attr_names, root_module_path
        )
        ...
        if str(grandparent_dir).startswith(str(root_module_path)):
            ...
            if grandparent_file_path.exists():
                yield from walk_parent_modules(
                    grandparent_file_path, attr_names, root_module_path
                )


def prepare_module_paths(
    file_path: Path, root_module_path: Path | None
) -> tuple[Path, Path]:
    """
...
def walk_module_attr(
    file_path: Path, attr_names: str | list[str], root_module_path: Path | None = None
) -> Iterator[ModuleVarSpec]:
    """
...
def walk_module_with_special_files(
    ...
    logger.debug(
        f"Built module hierarchy with {len(module_hierarchy.module_paths)} paths"
    )
    ...
    if not has_yielded_results:
        logger.info(
            f"No attributes named {attr_names_list} found in any module or special file"
        )
