package com.cyberagent.ailab.pinjectdesign.helpers
...
import com.cyberagent.ailab.pinjectdesign.config.PyConfiguration
import com.cyberagent.ailab.pinjectdesign.config.ConfigurationWrapper
...
class InjectedFunctionActionHelper(val project: Project) {
    ...
    fun cachedConfigurations(name: String): Promise<List<PyConfiguration>> {
        return runInBackground("Find Configurations") { indicator ->
            ...
            var configs = com.cyberagent.ailab.pinjectdesign.config.InjectedFunctionActionHelperObject.cache.getOrCreate(filePath) { key ->
                loaded = true
            ...
            if (!configs.containsKey(name) && !loaded) {
                ...
                com.cyberagent.ailab.pinjectdesign.config.InjectedFunctionActionHelperObject.cache[filePath] = configs
            }
    ...
    fun createRunActionsCached(name: String): Promise<List<com.cyberagent.ailab.pinjectdesign.lineMarkers.ActionItem>> {
        return cachedConfigurations(name).then { configs ->
            configs.map { conf ->
                com.cyberagent.ailab.pinjectdesign.lineMarkers.ActionItem(conf.name) {
                    runConfig(conf)
                }
            }
        }
    ...
    fun updateConfigurations() {
        val filePath = getFilePath() ?: throw IllegalStateException("File not found")
        val confs = findConfigurations(filePath)
        com.cyberagent.ailab.pinjectdesign.config.InjectedFunctionActionHelperObject.cache[filePath] = confs
    }
}
